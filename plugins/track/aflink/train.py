...
from dataset import LinkData, DartfishLinkData

def train(save: bool):
    model = PostLinker()
    model.cuda()
    model.train()
    dataset = DartfishLinkData(cfg.root_train, 'train')
    ...
    if save:
        ...
        torch.save(model.state_dict(), join(cfg.model_savedir, 'dartfishmodel_epoch{}.pth'.format(epoch + 1)))
    return model

def validate(model):
    model.eval()
    dataset = DartfishLinkData(cfg.root_train, 'test')
    dataloader = DataLoader(
        dataset=dataset,
